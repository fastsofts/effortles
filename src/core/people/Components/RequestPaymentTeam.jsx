...
import { MultipleSelect } from '@components/Select/Select.jsx';
...
import css from '../../Payables/Ageing/Ageing.scss';
...
const RequestPayment = (props) => {
  const { entity_id, setRequestPayment, sentEmail } = props;
  ...
  const fetchContactByCustomer = (s_ids) => {
    ...
    RestApi(
      `organizations/${organization.orgId}/vendors/${entity_id}/contacts`,
      {
        method: METHOD.GET,
        headers: {
  ...
  const handleClick = () => {
    ...
    enableLoading(true);
    RestApi(`organizations/${organization.orgId}/entity_follow_up_email`, {
      method: METHOD.POST,
      payload: {
        ...params,
        entity_id,
  ...
  return (
    <div className={css.bodyContent}>
      {device === 'mobile' ? (
        <div className={css.valueHeader}>Contact Vendor</div>
      ) : (
        <></>
      )}
      <div
