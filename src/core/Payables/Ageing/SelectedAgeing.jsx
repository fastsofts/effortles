...
import RequestPayment from '@core/Payables/Ageing/RequestPayment.jsx';
...
const SelectedAging = () => {
  ...
  React.useEffect(() => {
    ...
    if (
      Object.keys(userRoles?.['Vendor Analytics'] || {})?.length > 0 &&
      menuSelected === 'Analytics'
    ) {
      if (!userRoles?.['Vendor Analytics']?.view_payable_analystics) {
        setHavePermission({
          open: true,
          back: () => {
            setHavePermission({ open: false });
    ...
    if (
      Object.keys(userRoles?.['Vendor Relationships'] || {})?.length > 0 &&
      menuSelected === 'relationship'
    ) {
      if (!userRoles?.['Vendor Relationships']?.view_vendor_interactions) {
        setHavePermission({
          open: true,
          back: () => {
            setHavePermission({ open: false });
    ...
    userRoles?.['Vendor Analytics'],
    userRoles?.['Vendor Relationships'],
    menuSelected,
  ]);
  ...
  React.useEffect(() => {
    ...
    if (state?.from === 'people' || state?.from === 'payments') {
      setMenuSelected(state?.open);
    }
  }, [state]);

  const ProfilesSelection = (id) => {
    enableLoading(true);
    RestApi(`organizations/${organization.orgId}/accounts/${id}/profiles`, {
      method: METHOD.GET,
      headers: {
  ...
  const statement = (id) => {
    ...
    RestApi(
      `organizations/${
        organization.orgId
      }/payables/account_statements/${id}?start_date=${
        dateState.startDate === ''
          ? ''
  ...
  const SelectedOpenbillsCall = (id) => {
    enableLoading(true);
    RestApi(
      `organizations/${
        organization.orgId
      }/payables/open_bills?vendor_id=${id}&date=${moment(
        anchorElDate.value,
      ...
      if (res && !res.error) {
        setTopVal({
          collection: res?.payment_effectiveness,
          // dso: res?.dso,
          dpo: res?.dpo,
  ...
  React.useEffect(() => {
    SelectedOpenbillsCall(customerId);
  }, [anchorElDate?.value]);
    ...
    fetch(
      selected === 1
        ? `${BASE_URL}/organizations/${
            organization.orgId
          }/payables/open_bills.xlsx?vendor_id=${customerId}&date=${moment(
            anchorElDate.value,
          ).format('YYYY-MM-DD')}&report_view=${state?.wise}`
        : `${BASE_URL}/organizations/${
            organization.orgId
          }/payables/account_statements/${customerId}.xlsx?start_date=${
            dateState.startDate === ''
              ? ''
              : moment(dateState.startDate).format('YYYY-MM-DD')
          }&end_date=${
      ...
      >
        ...
        >
          ...
          >
            ...
            {device === 'desktop' && (
              <div className={css.secondBoxPaya}>
                {/* <div className={css.innerDiv1}>
                  ...
                  <div style={{ width: '75%' }}>
                    <p className={css.headPTag}>Total Outstanding</p>
                ...
                <div className={css.innerDiv2}>
                  ...
                  <div style={{ width: '75%' }}>
                    <p className={css.headPTag}>Current Outstanding</p>
                    <p className={css.subPTag}>
                      {FormattedAmount(topVal?.balance)}
                    </p>
                  </div>
                ...
                <div className={css.innerDiv3}>
                  ...
                  <div style={{ width: '75%' }}>
                    <p className={css.headPTag}>DPO</p>
                    <p className={css.subPTag}>{topVal?.dpo} Days</p>
                  </div>
                </div>
              </div>
            )}
            {device === 'mobile' && (
              <div className={css.newMobTopCard}>
                {[
                  ...
                  // { name: 'Collection Effectiveness', value: `${topVal?.collection}%`, color: '#F3894E' },
                  {
                    name: 'DPO',
                    value: `${topVal?.dpo} Days`,
                    color: '#ECBA3A',
                  },
                ].map((val) => (
          ...
          {/* )} */}
          <div className={device === 'mobile' ? css.payables : css.payables2}>
            {device === 'mobile' && (
            ...
            style={{
              borderRadius: device === 'mobile' ? '8px' : '0 10px 10px 0',
            }}
          ...
          >
            ...
            {menuSelected === 'openbills' && (
              ...
              >
                {device === 'desktop' && (
                  <div className={css.newChangeTabPaya}>
                    <div
            ...
            >
              ...
              {menuSelected === 'Agreements' && <Agreement id={customerId} />}
            </div>
          </div>
          {device === 'desktop' && (
            <PopperComp
